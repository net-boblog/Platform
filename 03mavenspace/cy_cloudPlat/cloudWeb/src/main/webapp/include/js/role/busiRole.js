...
$(function() {
	$('#dg').datagrid({  
		 loadMsg: "数据加载中，请稍后...",
		 url:path+'/role/system_queryBusiRoleList.pt', 
...
function saveOrUpdate(){
	...
	if(rolename !=null && rolename != ""){
		$('#f_form').form('submit', {    
		    url:path+'/role/system_saveOrUpdate.pt?type=busiRole&parentId=00', 
...
function delRole(){
	...
	if(row == null || row == ""){
		...
		$.post(path+"/role/system_delRole.pt",{id:row.id},function(data){
			 if(data=="success"){
