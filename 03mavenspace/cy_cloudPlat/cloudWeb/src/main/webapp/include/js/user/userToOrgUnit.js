...
$(function() {
     $('#tt').tree({     
	    url:path+'/orgunit/system_orgUnitTree.pt',
...
function setInfo(node){
	$.post(path+"/orgunit/system_getOrgUnit.pt",{id:node.id},function(data){
		var obj = eval("("+data+")"); 
		$('#name').val(obj.name);
		...
		$('#dg').datagrid({  
			 loadMsg: "数据加载中，请稍后...",
			 url:path+'/user/system_queryUserByOrgUnit.pt?orgId='+obj.id, 
			 loadFilter : pagerFilter
		});
	});
}
...
function openWin(){
	...
	if(orgId !=null && orgId != ""){
		...
		$('#win_grid').datagrid({  
			 loadMsg: "数据加载中，请稍后...",
			 url:path+'/user/system_queryOtherOrgUser.pt?orgId='+orgId, 
			 loadFilter : pagerFilter
		});
	}else{
...
function selectUser(){
	...
	if(rows!=null && rows != ""){
		...
		$.post(path+"/user/system_saveOrgUnitUser.pt",{ids:ids,orgId:orgId},function(data){
			$('#dg').datagrid('reload');
		});
	}
	$('#win').window('close');
...
function delOrgUnitUser(){
	...
	if(rows.length <= 0){
		...
		$.post(path+"/user/system_delOrgUnitUser.pt",{ids:ids},function(data){
			if(data=='success'){
