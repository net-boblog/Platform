...
function addUser(type){
	...
	$('#win').window({    
	    ...
	    top:2,
	    cache:true,
	    href:path+'/user/system_addOrUpdate.pt?id='+id,
...
function saveOrUpdate(){
	...
	if(account==null || account==""){
		...
		$('#f_form').form('submit', {    
		    url:path+'/user/system_saveOrUpdate.pt', 
...
function delUser(){
	...
	if(rows.length <= 0){
		...
		$.post(path+"/user/system_delUser.pt",{ids:ids},function(data){
			if(data=='success'){
...
function doStatus(enabled){
	...
	$.post(path+"/user/system_updateStatus.pt",{id:id,enabled:enabled},function(data){
		if(data=='success'){
			if(enabled == '1'){
...
function doLock(locked){
	...
	$.post(path+"/user/system_updateLock.pt",{id:id,locked:locked},function(data){
		if(data=='success'){
			if(locked == '1'){
