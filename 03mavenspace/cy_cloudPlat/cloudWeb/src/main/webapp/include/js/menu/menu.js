...
$(function() {
	$('#tt').tree({     
	    url:path+'/menu/system_menuTree.pt',
	    cascadeCheck: false,
...
function setInfo(node){
	$.post(path+"/menu/system_getMenuInfo.pt",{id:node.id},function(data){
		var obj = eval("("+data+")"); 
		$('#name').val(obj.name);
		$('#url').val(obj.url);
...
function delMenu(){
	var node = $('#tt').tree('getSelected');
	$.post(path+"/menu/system_delMenu.pt",{id:node.id},function(data){
			 if(data=="success"){
				 alert("删除成功！");
			 }else{
				 alert("删除失败！");
...
function saveOrUpdate(){
	var id = $('#menuId').val();
	$('#f_xm').form('submit', {    
	    url:path+'/menu/system_saveOrUpdate.pt', 
